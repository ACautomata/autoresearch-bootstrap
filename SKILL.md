...
**Step 2 — Fill the config.** Read [workflow-template.js](references/workflow-template.js) — it is the *blueprint* (Setup → Baseline → a Plan/Run/Analyze loop where each Plan agent is fed the stacked results of ALL prior experiments), not a file to save. Fill every `{PLACEHOLDER}` in the `C` object from Phases 1–2 + Step 1; the template's trailing "Template Variable Reference" comment documents each field. Three knobs need explicit user confirmation:
...
- **Experiment lifecycle ≠ agent lifecycle**: the loop is three SHORT agents per iteration, all decoupled from the training's wall-clock. A **Plan** agent stacks ALL prior experiment results (a `history` array seeded with the baseline) and uses that to pick/invent the next idea and modify train.py-scope code (snapshots pre-experiment HEAD for rollback). A **Run** agent commits, launches the self-terminating training, and awaits ONE completion notification (safety-net killing anything still alive past the deadline). An **Analyze** agent extracts metrics, decides keep/discard, MAINTAINS GIT (rollback on discard/crash), appends results.tsv, and writes a short analysis stacked into the next Plan prompt. Each training run is a self-terminating background process - kill-bounded by `maxTime` (a `timeout`/wall-time wrapper so the process dies at the deadline, NOT an agent watching a clock deciding to kill), awaited via ONE Bash `run_in_background` completion notification (never poll, never spawn check-agents). So no reasoning agent is held alive for the training duration: Plan and Analyze are short reasoning agents that run before/after the bounded process, and `maxTime` is enforced by the process itself (killed-at-deadline → crash). While the whole workflow runs in the background, the Workflow tool notifies you on completion - don't keep spawning check-agents; at most start ONE Monitor on `results.tsv` / the run log.
